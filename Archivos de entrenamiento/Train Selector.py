...
data_entrenamiento = './Selector/Entrenar' #Carpeta de entrenamiento
data_validacion = './Selector/Validar' #Carpeta de validacion

"""
Parameters
"""
epocas=12 #Epocas de entrenamiento
longitud, altura = 200, 200 #Nuevos tama;ios de las imagenes
batch_size = 32 #Numero de imagenes en cada paso
pasos = 100 #Pasos de cada epoca
validation_steps = 50 #200 pasos de validacion en cada epoca
...
tamano_filtro2 = (3, 3) #Tama;io del filtro
tamano_pool = (2, 2) #tama;o del max puling

clases = 3 #Clases por clasificar
lr = 0.0005 #LearingRange ajuste de error
...
target_dir = './Entrenamiento Selector/' #Lugar donde guardar
if not os.path.exists(target_dir): #Si no existe la carpeta llamada modelo
  os.mkdir(target_dir) #Crea la carpeta 
cnn.save('./Entrenamiento Selector/modelo.h5') #Direccion donde guardar el modelo con nombre "modelo.h5"
cnn.save_weights('./Entrenamiento Selector/pesos.h5') #Direccion para guardar los pesos
